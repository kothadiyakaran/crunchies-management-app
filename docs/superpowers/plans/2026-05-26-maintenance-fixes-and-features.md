...
- **Task #6 — DONE** (`3e5b495`). events-flow flake root-caused (fixed phone → stale-customer anti-leak null, not a timeout) + fixed: unique phone, self-cleaning, 20s wait + diagnostic, `--url`.
- **Task #7 — SHIPPED** (`226048f`/`1423c27`). Repeat-customer cross-event "Order not found." fixed via `orders.event_id` (migration 0009) + RPC anti-leak rekey; `verify-exhibition-repeat.py` (incl. anti-leak invariant). The bug task #6 uncovered.
- **Part E — DONE.** CLAUDE.md (`npm run test:run` fix, blast-radius smoke cadence, migration range 0001-0009, data-model + architecture pointers for discounts/reversibility/canvas-bill/event_id, status), `docs/v1-spec.md` (§2 columns + §14 Phase 2 shipped list + header), `.gitignore` (__pycache__). **All maintenance work complete.**
