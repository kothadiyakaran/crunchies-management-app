...
**Phase 1 status (2026-05-22):** Sprints 0–6 complete and deployed to `https://www.crunchies.app`. Detail (deliverables, test counts, decisions) lives in `CLAUDE.md` under "Phase 1 status" and in `docs/decisions/*-architecture-decisions.md`. Sprint 7 (Events + customer-facing exhibition form + confirmation route) is the next coding move; the next review checkpoint is the Sprint 7+8 pairing per the agreed three-pair structure.
