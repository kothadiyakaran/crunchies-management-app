...
Smoke at `scripts/smoke-test-walking-skeleton.py` (re-runnable; reads `SMOKE_EMAIL` / `SMOKE_PASSWORD` from process env or `.env.local`, supports both Vite `KEY=value` and PowerShell `$env:KEY = "value"` forms). Test count: 206 across 32 vitest files.

Sprint 9 (Settings + onboarding + accessibility / empty-state polish + algorithm event-uplift wire-up + real-data backfill — §13 fields + `/settings` UI, `BUSINESS_INFO` constants → Settings-table read, `production/algorithm.ts:event_uplift` consumption from `event_demand`, accessibility pass, performance pass on mom's phone, idempotent backfill script for mom's notebook history) is the next coding move per the §14 sequence. Then Sprint 10 (QA — walk all 8 §3 flows with synthetic data then with backfilled real data; cross-browser smoke) and the in-person launch session. **Open Sprint 0 carry**: install + launch verified on mom's actual Android (§14 line 1538) — still pending; Karan can do this asynchronously by opening `crunchies.app` on her phone and installing to home screen. Architectural decisions for Sprint 8 captured in `docs/decisions/2026-05-22-sprint-8-architecture-decisions.md`; Sprint 7 in `docs/decisions/2026-05-22-sprint-7-architecture-decisions.md`; Sprint 6 in `docs/decisions/2026-05-22-sprint-6-architecture-decisions.md`; Sprint 5 in `docs/decisions/2026-05-22-sprint-5-architecture-decisions.md`; Sprint 3-4 in `docs/decisions/2026-05-21-sprint-3-4-architecture-decisions.md`; Sprint 2 in `docs/decisions/2026-05-21-sprint-2-architecture-decisions.md`.
