...
**Phase 1 status:** Sprints 0–7 complete and deployed to `https://www.crunchies.app` (2026-05-22).
...
Smoke at `scripts/smoke-test-walking-skeleton.py` (re-runnable; reads `SMOKE_EMAIL` / `SMOKE_PASSWORD` from process env or `.env.local`, supports both Vite `KEY=value` and PowerShell `$env:KEY = "value"` forms). Test count: 128 across 23 vitest files.

Sprint 8 (Reports — Week / Month / Trends tabs with calibration card, plan-accuracy line chart, per-product sparklines, channel mix trend, past-event retrospectives) is the next coding move per the §14 sequence. Architectural decisions for Sprint 7 captured in `docs/decisions/2026-05-22-sprint-7-architecture-decisions.md`; Sprint 6 in `docs/decisions/2026-05-22-sprint-6-architecture-decisions.md`; Sprint 5 in `docs/decisions/2026-05-22-sprint-5-architecture-decisions.md`; Sprint 3-4 in `docs/decisions/2026-05-21-sprint-3-4-architecture-decisions.md`; Sprint 2 in `docs/decisions/2026-05-21-sprint-2-architecture-decisions.md`.
